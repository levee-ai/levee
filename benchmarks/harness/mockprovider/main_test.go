...
// TestServe_StreamsAreRepeatable serves each stream twice in a row and holds
// both responses against the fixture. A replay that consumed or rewrote the
// loaded buffer would diverge on the second pass.
func TestServe_StreamsAreRepeatable(t *testing.T) {
	fixtures := loadTestFixtures(t)
	server := httptest.NewServer(newHandler(fixtures))
	defer server.Close()
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			for pass := 1; pass <= 2; pass++ {
				response, err := http.Post(server.URL+testCase.path, contentTypeJSON, strings.NewReader(testCase.body))
				if err != nil {
					t.Fatalf("pass %d post: %v", pass, err)
				}
				body := readAll(t, response)
				if err := response.Body.Close(); err != nil {
					t.Fatalf("pass %d close: %v", pass, err)
				}
				if !bytes.Equal(body, testCase.want) {
					t.Errorf("pass %d stream is not the fixture bytes: got %d bytes, want %d", pass, len(body), len(testCase.want))
				}
			}
		})
	}
...
func TestServe_MalformedBodyIsRejected(t *testing.T) {
	...
	response, err := http.Post(server.URL+"/v1/chat/completions", contentTypeJSON, strings.NewReader("not json"))
	if err != nil {
		t.Fatalf("post: %v", err)
	}
	defer func() { _ = response.Body.Close() }()
...
func TestServe_HealthzIsReady(t *testing.T) {
	fixtures := loadTestFixtures(t)
	server := httptest.NewServer(newHandler(fixtures))
	defer server.Close()
